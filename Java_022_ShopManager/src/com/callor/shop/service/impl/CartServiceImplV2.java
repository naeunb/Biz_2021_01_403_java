...
/*
 * CartServiceImplV1을 extends(확장,상속)받아서 V2생성.
 */
public class CartServiceImplV2 extends CartServiceImplV1 implements CartServiceV2 {
	...
	public void saveCartToFile() {
		...
		//PrintWriter클래스를 사용하여 객체선언
		PrintWriter printer = null;
		
		/*
		 * 3.fileWriter객체를 생성하면서 데이터를 저장할 파일정보를 전달.
		 * => 운영체제와 통신을 하여 파일에 저장할 준비를 한다. 이때 운영체제와 통신을 시작하는
		 * 	  아주 짧은 시간동안 생성하고자 하는 파일의 각종 정보들에 문제가 발생하여 exception이
		 *    발생할 수 있다.
		 *    
		 *    아무리 코드를 견고하게 작성해도 이 exception은 완전하게 피할 수 없음.
		 *    
		 *    java에서는 이처럼 완전하게 exception을 피할 수 없는 경우에는 반드시 try 예외처리를 
		 *    하도록 강제한다.
		 *    따라서 FileWriter 객체를 생성할때는 반드시 try..catch로 묶어야한다.
		 */
		try {
			fileWriter = new FileWriter(cartFileName);
			//여기까지 코드가 정상수행되면
			//fileWriter는 System.in처럼 파일과 java코드간에 연결하는 통로를 형성한다.
